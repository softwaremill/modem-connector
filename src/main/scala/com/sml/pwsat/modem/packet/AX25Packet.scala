...
class AX25Packet {
  private val AX25_CRC_CORRECT: Int = 0xF0B8
  private val CRC_CCITT_INIT_VAL: Int = 0xFFFF
  /**
    * int MAX_FRAME_SIZE = // not including delimiting flags
    ...
    * +2;             // frame checksum
    */
  private val MAX_FRAME_SIZE: Int = 7 + 7 + (8 * 7) + 1 + 1 + 256 + 2


  var size: Int = 0
  var crc: Int = CRC_CCITT_INIT_VAL
  val packet: Array[Byte] = new Array[Byte](MAX_FRAME_SIZE)


  def terminate(): Boolean = {
    size >= 18 && (crc == AX25_CRC_CORRECT)
  }

  def addByte(b: Byte): Boolean = {
  ...
  def bytesWithoutCRC(): Array[Byte] = packet.slice(0, size - 2)

  private def crcCcittUpdate(b: Byte) {
    crc = (crc >> 8) ^ AX25Packet.CrcCcittTab((crc ^ b) & 0xff)
  }
}
...
object AX25Packet {
  ...
  private def parseCall(packet: Array[Byte], offset: Int): String = {
    val CallSize: Int = 6
    def parseChar(position: Int): Char = {
      if (packet(offset + position) > 0) {
        (packet(offset + position) >> 1).toChar
      } else {
        ((packet(offset + position) + 256) >> 1).toChar
      }
    }

    def parseCall(acc: Int): String = {
      if (acc < CallSize) {
        val ch = parseChar(acc)
        if (ch != ' ') {
          ch + parseCall(acc + 1)
        } else {
          parseCall(acc + 1)
    ...
    def parseSign(): String = {
      val ssid: Int = parseChar(CallSize) & 0x0f
      if (ssid != 0) {
        "-%d" format ssid
      } else {
        ""
    ...
    parseCall(0) + parseSign()
  }

  def format(packet: Array[Byte]): String = {
    var offset: Int = 0

    val destination: String = parseCall(packet, offset)
    offset += 7
    val source: String = parseCall(packet, offset)
    offset += 7
    ...
    var continue = true
    while (offset + 7 <= size && (packet(offset - 1) & 0x01) == 0 && continue) {
      repeaters += 1
      if (repeaters > 8) {
        continue = false
      } else {
        // missing LSB=1 to terminate the path
        val path_element: String = parseCall(packet, offset)
        offset += 7
        path match {
          case Array(x, _*) =>
            path = path.slice(0, path.length + 1)
            path(path.length - 1) = path_element
          case _ => path = Array(path_element)
        }
      }
    }
    offset += 2
    // skip PID, control
    val payload = packet.slice(offset, size)
    buildContent(source, destination, path, payload)
  }

  private def buildContent(source: String, destination: String, path: Array[String], payload: Array[Byte]): String = {
    ...
    def getPayloadChars(p: Array[Byte]): String = {
      p match {
        case Array(x, tail@_*) =>
          if (x >= 0x20 && x <= 0x7E) {
            x.toChar + getPayloadChars(tail.toArray)
          } else {
            ("\\x%02x" format x) + getPayloadChars(tail.toArray)
          }
        case Array() => ""
      }
    }
    builder.append(getPayloadChars(payload))
    builder.append("]")
    builder.toString()
  }
  ...
  private val CrcCcittTab: Array[Int] = Array(0x0000, 0x1189, 0x2312, 0x329b, 0x4624, 0x57ad, 0x6536, 0x74bf, 0x8c48,
